...
export const MessengerWindow: FC = () => {
  ...
  const messeges = useCallback(() => {
    const recieved = chats.find((chat) => chat.current === true)?.messages
      .received;
    const sended = chats.find((chat) => chat.current === true)?.messages.sended;
            ...
            <SendedMessage key={message.id} $type={message.type}>
              ...
              <CheckContainer>
                {message.status === "sent" ? (
                  ...
                  message.status === "read" ? (
                  <DoneAll messageStatus={message.status}></DoneAll>
                ) : (
  ...
  return (
    <Wrapper>
      ...
      <ChatsColumn>
        <ChatHeader $position="chatsList"></ChatHeader>
        <PhoneForm
          onSubmit={(e) => {
            e.preventDefault();
        ...
        >
          <Input
            ...
            value={phoneInput}
            onChange={(e) => dispatch(changePhoneInput(e.target.value))}
          ></Input>
        </PhoneForm>
